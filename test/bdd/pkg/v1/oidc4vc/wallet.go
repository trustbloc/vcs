...
func (s *Steps) createWallet() error {
	s.walletUserID = "testUserID" + uuid.NewString()
	s.walletPassphrase = "passphrase122334"

	services, err := CreateAgentServices(s.tlsConfig)
	if err != nil {
	...
	w, err := NewWallet(s.walletUserID, s.walletPassphrase, s.ariesServices)
	if err != nil {
		return err
	}
	s.wallet = w

	token, err := s.wallet.Open(wallet.WithUnlockByPassphrase(s.walletPassphrase))
	if err != nil {
	...
	vdr, err := orb.New(nil, orb.WithDomain(didDomain), orb.WithTLSConfig(s.tlsConfig),
	...
	s.walletDidID = createRes.DidID
	s.bddContext.CredentialSubject = createRes.DidID
	s.walletDidKeyID = createRes.KeyID

	for i := 1; i <= vdrResolveMaxRetry; i++ {
		_, err = vdrRegistry.Resolve(s.walletDidID)
		println("walletDidID", s.walletDidID)
		if err == nil {
			break
		}
...
func (s *Steps) saveCredentialsInWallet() error {
	for _, cred := range s.bddContext.CreatedCredentialsSet {
		err := s.wallet.Add(s.walletToken, wallet.Credential, cred)
		if err != nil {
