...
Feature: Using VC REST API
  ...
  Scenario Outline: Store, retrieve, verify credential and presentation using different kind of profiles
    Given Profile "<profile>" is created with DID "<did>", privateKey "<privateKey>", signatureHolder "<signatureHolder>", uniRegistrar '<uniRegistrar>' and didMethod "<didMethod>"
    And   We can retrieve profile "<profile>" with DID "<didMethod>"
    ...
    Examples:
      | profile              | credential                   | did                                                              | privateKey                                                                               | signatureHolder |                           uniRegistrar                                                                                                                        |  didMethod      |
      | myprofile_ud         | university_degree.json       |                                                                  |                                                                                          | ProofValue      |       {"driverURL":"http://uni-registrar-web:9080/1.0/register?driver-did-method-rest"}                                                                       | did:trustbloc   |
      | myprofile_prc        | permanent_resident_card.json |                                                                  |                                                                                          | ProofValue      |                                                                                                                                                               | did:trustbloc   |
      | myprofilewithdidv1   | university_degree.json       |                                                                  |                                                                                          | JWS             |       {"driverURL":"http://uni-registrar-web:9080/1.0/register?driver-universalregistrar/driver-did-v1","options": {"ledger": "test", "keytype": "ed25519"}}  | did:v1:test:nym |
      | myprofilewithdidelem | university_degree.json       | did:elem:EiAWdU2yih6NA2IGnLsDhkErZ8aQX6b8yKt7jHMi-ttFdQ          | 5AcDTQT7Cdg1gBvz8PQpnH3xEbLCE1VQxAJV5NjVHvNjsZSfn4NaLZ77mapoi4QwZeBhcAA7MQzaFYkzJLfGjNnR | JWS             |                                                                                                                                                               | did:elem        |
      | myprofilewithdidsov  | university_degree.json       |                                                                  |                                                                                          | JWS             |       {"driverURL":"https://uniregistrar.io/1.0/register?driver-universalregistrar/driver-did-sov","options": {"network":"danube"}}                           | did:sov:danube  |
