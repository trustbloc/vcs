...
Feature: Using OIDC4VP REST API
  ...
  Background:
    ...
    And   New verifiable credentials is created from table:
      ...
      | i_myprofile_ud_es384_sdjwt/v1.0 | test_org     | crude_product.json              | jwt_vc_json-ld | 0        |
    And User saves credentials into wallet


  Scenario: Initiate, check authorization response for ldp verifier with specific presentation definition ID and fields
  ...
  Scenario Outline: Initiate, check authorization response for jwt verifier
    Given OIDC4VP interaction initiated under "v_myprofile_jwt/v1.0" profile for organization "test_org" with presentation definition ID "<presentationDefinitionID>" and fields "<fields>"
    And Wallet verify authorization request and decode claims
    And Wallet looks for credential that match authorization
    And Wallet send authorization response
    And Verifier form organization "test_org" requests interactions claims
