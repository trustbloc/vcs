...
Feature: CHAPI Workflow - API only
  ...
  Scenario Outline: CHAPI Workflow - API only (https://github.com/w3c-ccg/vc-examples/tree/master/docs/chapi-http-edu)
    ...
    When  "<verifierProfile>" sends DIDAuth request to "<holderProfile>" for authentication
    And   "<holderProfile>" sends response to DIDAuth request from "<verifierProfile>"
    Then  "<verifierProfile>" verifies the DIDAuth response from "<holderProfile>"
    Then  "<verifierProfile>" verifies the "<flow>" presented by "<holderProfile>"
    Examples:
      | flow                        | issuerProfile         | holderProfile     | verifierProfile        | credential                       | issuerKeyType | issuerSignType        | holderKeyType | holderSignType          |
      | Education Degree            | university            | alice             | acme                  | university_degree.json           | Ed25519       | Ed25519Signature2018  | Ed25519       | Ed25519Signature2018    |
      | Permanent Resident Card     | government            | bob               | immigrationDept1       | permanent_resident_card.json     | Ed25519       | Ed25519Signature2018  | Ed25519       | Ed25519Signature2018    |
      | Permanent Resident Card     | government_ed25519    | bob_p256          | immigrationDept2       | permanent_resident_card.json     | Ed25519       | Ed25519Signature2018  | P256          | JsonWebSignature2020    |
