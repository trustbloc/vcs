...
func (s *Service) VerifyCredential(credential *verifiable.Credential, opts *VerifyCredentialOptions,
	...
	if checks.Proof {
		...
		if err != nil {
			return nil, fmt.Errorf("unexepcted error on credential marshal: %w", err)
		}
...
func (s *Service) validateCredentialProof(vcByte []byte, opts *VerifyCredentialOptions, vcInVPValidation bool) error { // nolint: lll,gocyclo
...
func (s *Service) checkVCStatus(vcStatus *verifiable.TypedID, issuer string) (*VerificationStatus, error) {
	...
	/* TODO: this code should be in implementation of vcStatusManager.GetRevocationListVC
