...
func (w *Wrapper) CreateStatusListEntry(
	...
	span.SetAttributes(attribute.String("status_purpose", statusPurpose))

	entry, err := w.svc.CreateStatusListEntry(ctx, profileID, profileVersion, credentialID, statusPurpose)
	if err != nil {
		return nil, err
	}
