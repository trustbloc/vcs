...
func TestProfileStore_CreateFails(t *testing.T) {
	pool, mongoDBResource := startMongoDBContainer(t)
...
func TestProfileStore_UpdateFails(t *testing.T) {
	pool, mongoDBResource := startMongoDBContainer(t)
	...
	t.Run("Update invalid profile id", func(t *testing.T) {
		err := store.Update(&verifier.Profile{ID: "invalid"})
		require.Contains(t, err.Error(), "verifier profile invalid id")
	})

	t.Run("Update not existing profile id", func(t *testing.T) {
		err := store.Update(&verifier.Profile{ID: "121212121212121212121212"})
		require.Contains(t, err.Error(), "profile with given id not found")
	})
}

func TestProfileStore_FindFails(t *testing.T) {
	pool, mongoDBResource := startMongoDBContainer(t)
