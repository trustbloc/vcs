...
func CheckAll(t *testing.T, provider storage.Provider) {
	...
	t.Run("Provider: open store and set/get config", func(t *testing.T) {
		ProviderOpenStoreSetGetConfig(t, provider)
	})
	t.Run("Store", func(t *testing.T) {
		...
		t.Run("GetTags", func(t *testing.T) {
			StoreGetTags(t, provider)
		})
		t.Run("GetBulk", func(t *testing.T) {
			StoreGetBulk(t, provider)
		})
		t.Run("Delete", func(t *testing.T) {
			StoreDelete(t, provider)
		})
		t.Run("Query", func(t *testing.T) {
			StoreQuery(t, provider)
			StoreQueryWithSortingAndInitialPageOptions(t, provider)
		})
		t.Run("Batch", func(t *testing.T) {
			StoreBatch(t, provider)
		})
		t.Run("Flush", func(t *testing.T) {
			StoreFlush(t, provider)
		})
		t.Run("Close", func(t *testing.T) {
			StoreClose(t, provider)
		})
	})
...
func ProviderOpenStoreSetGetConfig(t *testing.T, provider storage.Provider) { //nolint: funlen // Test file
	t.Run("Set store config with all new tags", func(t *testing.T) {
		...
		config := storage.StoreConfiguration{TagNames: []string{"tagName1", "tagName2", "tagName3"}}
	...
	t.Run("Merge a new tag name in with existing tag names in a store config", func(t *testing.T) {
		...
		// Set initial tags.
		err = provider.SetStoreConfig(storeName, storage.StoreConfiguration{TagNames: []string{"tagName1", "tagName2"}})
		require.NoError(t, err)
	...
	t.Run("Merge a new tag in with existing tags while deleting some too", func(t *testing.T) {
		...
		err = provider.SetStoreConfig(storeName,
			storage.StoreConfiguration{TagNames: []string{"tagName1", "tagName2"}})
		require.NoError(t, err)

		// Now we want tagName1 to be removed, tagName2 to be kept, and tagName3 to be added.
		err = provider.SetStoreConfig(storeName,
			storage.StoreConfiguration{TagNames: []string{"tagName2", "tagName3"}})
		require.NoError(t, err)
	...
	t.Run("Attempt to set config without opening store first", func(t *testing.T) {
		err := provider.SetStoreConfig("NonExistentStore", storage.StoreConfiguration{})
	...
	t.Run("Attempt to set a config that specifies a tag name with a ':' character", func(t *testing.T) {
		...
		config := storage.StoreConfiguration{TagNames: []string{"tagName1", "tagName2", "tagNameWith:Character"}}
	...
	t.Run("Demonstrate that store names are not case-sensitive", func(t *testing.T) {
		...
		err = provider.SetStoreConfig("SoMe-stoRe-naMe", storage.StoreConfiguration{TagNames: []string{"TagName1"}})
		require.NoError(t, err)
...
// ProviderGetOpenStores tests common Provider GetOpenStores functionality.
...
func PutGet(t *testing.T, provider storage.Provider) { //nolint: funlen // Test file
...
func StoreGetTags(t *testing.T, provider storage.Provider) {
	...
	t.Run("Successfully retrieve tags", func(t *testing.T) {
		...
		t.Run("Tag values are strings", func(t *testing.T) {
			tags := []storage.Tag{{Name: "tagName1", Value: "tagValue1"}, {Name: "tagName2", Value: "tagValue2"}}
...
func StoreQuery(t *testing.T, provider storage.Provider) {
	doStoreQueryTests(t, provider, false)
	doStoreQueryTests(t, provider, true)
}

// StoreQueryWithSortingAndInitialPageOptions tests common Store Query functionality when the sorting and initial
// page options are used.
func StoreQueryWithSortingAndInitialPageOptions(t *testing.T, provider storage.Provider) {

	doStoreQueryWithSortingAndInitialPageOptionsTests(t, provider, false)

	doStoreQueryWithSortingAndInitialPageOptionsTests(t, provider, true)
}

// StoreBatch tests common Store Batch functionality.
func StoreBatch(t *testing.T, provider storage.Provider) { // nolint:funlen // Test file
	...
	t.Run("Success: update three different previously-stored values", func(t *testing.T) {
		...
		key1UpdatedTagsToStore := []storage.Tag{{Name: "tagName1"}}
		key2UpdatedTagsToStore := []storage.Tag{{Name: "tagName2_new", Value: "tagValue2"}}
		key3UpdatedTagsToStore := []storage.Tag{{Name: "tagName3_new", Value: "tagValue3_new"}}
	...
	t.Run("Success: put value and then delete it in the same Batch call", func(t *testing.T) {
		...
		err = provider.SetStoreConfig(storeName,
			storage.StoreConfiguration{TagNames: []string{"tagName1"}})
		require.NoError(t, err)
	...
	t.Run("Success: put value and update it in the same Batch call", func(t *testing.T) {
		...
		updatedTagsToStore := []storage.Tag{{Name: "tagName2", Value: "tagValue2"}}
	...
	t.Run("Success: update previously-stored value, then delete it, "+
		...
		key1UpdatedTagsToStore := []storage.Tag{{Name: "tagName1"}}
		key1SecondUpdatedTagsToStore := []storage.Tag{{Name: "tagName2"}}
	...
	t.Run("Success: put values in one batch call, then delete in a second batch call, then put again using "+
		...
		operations := []storage.Operation{
			{Key: "key1", Value: []byte("value1"), Tags: []storage.Tag{{Name: "tagName1", Value: "tagValue1"}}},
			{Key: "key2", Value: []byte("value2"), Tags: []storage.Tag{{Name: "tagName2", Value: "tagValue2"}}},
			{Key: "key3", Value: []byte("value3"), Tags: []storage.Tag{{Name: "tagName3", Value: "tagValue3"}}},
		}
		...
		key1FinalTagsToStore := []storage.Tag{{Name: "tagName1_new", Value: "tagValue1_new"}}
		key2FinalTagsToStore := []storage.Tag{{Name: "tagName2_new", Value: "tagValue2_new"}}
		key3FinalTagsToStore := []storage.Tag{{Name: "tagName3_new", Value: "tagValue3_new"}}
	...
	t.Run("Failure: Operations slice is empty", func(t *testing.T) {
		...
		err = store.Batch([]storage.Operation{})
		require.Error(t, err)
	})
...
func storeTestJSONData(t *testing.T, store storage.Store, key string) testStruct {
	testData := testStruct{
...
func doStoreQueryTests(t *testing.T, // nolint: funlen,gocognit,gocyclo // Test file
	provider storage.Provider, setStoreConfig bool) {
	t.Run("Tag name only query - 2 values found", func(t *testing.T) {
		...
		t.Run("Default page setting", func(t *testing.T) {
			...
			if setStoreConfig {
				err = provider.SetStoreConfig(storeName,
					storage.StoreConfiguration{TagNames: []string{"tagName1", "tagName2", "tagName3", "tagName4"}})
				require.NoError(t, err)
			}
	...
	t.Run("Tag name only query - 0 values found", func(t *testing.T) {
		...
		t.Run("Default page setting", func(t *testing.T) {
			...
			if setStoreConfig {
				err = provider.SetStoreConfig(storeName,
					storage.StoreConfiguration{TagNames: []string{"tagName1", "tagName2", "tagName3", "tagName4", "tagName5"}})
				require.NoError(t, err)
			}
	...
	t.Run("Tag name and value query - 2 values found", func(t *testing.T) {
		...
		expectedTags := [][]storage.Tag{tagsToPut[1], tagsToPut[3]}
		expectedTotalItemsCount := 2
...
func doStoreQueryWithSortingAndInitialPageOptionsTests(t *testing.T, // nolint: funlen // Test file
	...
	t.Run("Sorting by a small numerical tag", func(t *testing.T) { //nolint: dupl // Test file
		...
		tagsToPutAscendingOrder := [][]storage.Tag{
			{
				{Name: "tagName1", Value: "tagValue1"},
				{Name: "tagName2", Value: "tagValue2"},
		...
		t.Run("Data inserted in ascending order", func(t *testing.T) {
				...
				t.Run("Page size 3", func(t *testing.T) {
					t.Run("Start at the default (first) page", func(t *testing.T) {
						iterator, err := store.Query(queryExpression,
							...
							storage.WithPageSize(3)) // nolint: gomnd // Test file
						require.NoError(t, err)
					...
					t.Run("Start at third page", func(t *testing.T) {
						iterator, err := store.Query(queryExpression,
							...
							storage.WithPageSize(3),       // nolint: gomnd // Test file
							storage.WithInitialPageNum(2)) // nolint: gomnd // Test file
						require.NoError(t, err)
					...
					t.Run("Start at fifth page (but there should only be four pages max, "+
						...
						iterator, err := store.Query(queryExpression,
							...
							storage.WithPageSize(3),       // nolint: gomnd // Test file
							storage.WithInitialPageNum(4)) // nolint: gomnd // Test file
						require.NoError(t, err)
			...
			t.Run("Descending order", func(t *testing.T) {
				...
				t.Run("Default page size setting", func(t *testing.T) {
					t.Run("Start at the default (first) page", func(t *testing.T) {
						querySortOption := storage.WithSortOrder(&storage.SortOptions{
		...
		t.Run("Data inserted in arbitrary order", func(t *testing.T) {
			...
			tagsToPutArbitraryOrder := [][]storage.Tag{
...
func doBatchTestPutThreeValues(t *testing.T, provider storage.Provider, useNewKeyOptimization bool) {
	...
	key1TagsToStore := []storage.Tag{{Name: "tagName1"}}
	key2TagsToStore := []storage.Tag{{Name: "tagName2"}}
	key3TagsToStore := []storage.Tag{{Name: "tagName3"}}

	putOptions := &storage.PutOptions{IsNewKey: useNewKeyOptimization}
...
func doBatchTestPutOneUpdateOneDeleteOne(t *testing.T, provider storage.Provider, useNewKeyOptimization bool) {
	...
	key3TagsToStore := []storage.Tag{{Name: "tagName3", Value: "tagValue3"}}
...
func verifyExpectedIterator(t *testing.T, actualResultsItr storage.Iterator, expectedKeys []string, expectedValues [][]byte,
	expectedTags [][]storage.Tag, orderMatters, checkTotalItemsCount bool, expectedTotalItemsCount int) {
...
func verifyIteratorAnyOrder(t *testing.T, actualResultsItr storage.Iterator, //nolint: gocyclo,funlen // Test file
	expectedKeys []string, expectedValues [][]byte, expectedTags [][]storage.Tag,
...
func verifyIteratorInOrder(t *testing.T, actualResultsItr storage.Iterator,
	expectedKeys []string, expectedValues [][]byte, expectedTags [][]storage.Tag,
	checkTotalItemsCount bool, expectedTotalItemsCount int) {
	moreResultsToCheck, err := actualResultsItr.Next()
	require.NoError(t, err)
