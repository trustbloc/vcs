...
func MetricsMiddlewareWithConfig(config Config) echo.MiddlewareFunc {
	...
	}, []string{"status", "method", "handler", "version", "scope", "domain"})
	...
	}, []string{"method", "handler", "version", "scope", "domain"})
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(context echo.Context) error {
			...
			timer := prometheus.NewTimer(httpDuration.WithLabelValues(request.Method, path,
				config.Version, config.Scope, config.Domain))
			err := next(context)
			...
			httpRequests.WithLabelValues(status, request.Method, path, config.Version, config.Scope, config.Domain).
				Inc()
